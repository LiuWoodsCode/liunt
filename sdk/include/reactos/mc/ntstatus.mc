...
Language=English
{GUID Substitution}
During the translation of a global identifier (GUID) to a LiuNT security ID (SID), no administratively-defined GUID prefix was found.
...
{EXCEPTION}
Cannot Continue
LiuNT cannot continue from this exception.
.
...
Language=English
{Bad Image}
%hs is either not designed for LiuNT or it contains an error.
...
Language=English
The SAM database on a LiuNT Server is significantly out of synchronization with the copy on the Domain Controller. A complete synchronization is required.
.
...
Language=English
The NtCreateFile API failed. This error should never be returned to an application, it is a place holder for the LiuNT Lan Manager Redirector to use in its internal error mapping routines.
.
...
LiuNT was unable to save all the data for the file %hs. The data has been lost.
...
Language=English
{LiuNT Evaluation Notification}
LiuNT is free and open source; there is no licence to expire. You should not see this.
.
...
The relocation occurred because the DLL %hs occupied an address range reserved for LiuNT system DLLs. The vendor supplying the DLL should be contacted for a new DLL.
.
...
Language=English
The LiuNT I/O reparse tag passed for the NTFS reparse point is invalid.
.
...
Language=English
The LiuNT I/O reparse tag does not match the one present in the NTFS reparse point.
.
...
Your system is low on virtual memory. LiuNT is increasing the size of your virtual memory paging file.
...
Language=English
LiuNT was not able to process the application binding information.
